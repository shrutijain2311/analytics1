...
#2nd Method : import from CSV file
...
head(omni)
str(omni)
nrow(omni)#no. of rows
dim(omni)#no. of rows and columns
#MLR  Create Multiple Linear Regression
# we want to see how Sales Qty depend on Price and Promotion Values
fit2 = lm(sales ~ price + promotion, data=omni)#sale dependnent on price and promotion
...
fitted(fit2)#predicted values for input\actual price and promotion
omni$salesE#actual sales
residuals(fit2)#diff b\w predicted andactual sales
...
p2sales = predict(fit2, newdata=ndata2)#by default type=response so no need to type
cbind(ndata2, p2sales)
head(omni)
#Assumptions
par(mfrow=c(2,2))#par=multiple frames per row(2 rows and 2 columns)
plot(fit2)
par(mfrow=c(1,1))

plot(fit2,which=1)  # no pattern, equal variance.which= means which chart no.
plot(fit2,2)  # Residuals are normally distributed
plot(fit2,3)  # No hetero-scedascity.equal variants
plot(fit2,4)  # tells outliers which affect model.outliers are basically extreme values \exceptions to data
omni[c(11,14,15),]#row no. are being told
