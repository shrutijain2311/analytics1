...
library(arules)#used for assosciation mining in r
library(arulesViz)
library(datasets)


data('Groceries') #different format - transaction format
arules::inspect(Groceries[1:5])#pick up inspection func from arules package
df<-read.csv("Cosmetics.csv",header=TRUE, sep=",")#read the csv in file
head(df)

#apriori is used for creating rules
...
rules = apriori(as.matrix(df[,2:15]), parameter=list(support=0.15, confidence=0.65,minlen=2))#apriry from arules apckage
rulesc <- sort (rules, by="confidence", decreasing=TRUE)
inspect(rulesc[1:5])#buying lhs and rhs together# support:0.169 probability they are bought together (lhs and rhs).use html file to make sense.#higher the value better it is
                    #inspect is a kind of summary maker
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5))#min length of products purchased is 2 and max is 5
inspect(sort (frequentItems, by="count", decreasing=TRUE)[1:25])
dev.off()
itemFrequencyPlot(Groceries,topN = 15,type = 'absolute')#plotting top 15 values in yerms of number of occurences
...
inspect(subset2) # all items  %ain% #in means any of the value present and ain means both i.e is and 
